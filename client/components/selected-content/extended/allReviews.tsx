...
export default function AllReviews({
  ...
  function popularityProxy(r: Review) {
    return (r.author_details?.rating ?? 0) + Math.min(5, (r.content?.length ?? 0) / 200);
  }
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "highest":
        arr.sort((a, b) => {
          const ra = a.author_details?.rating ?? -1, rb = b.author_details?.rating ?? -1;
          return ra === rb
  ...
  const toggleExpanded = (id: string) =>
    setExpandedReviews((p) => { const s = new Set(p); s.has(id) ? s.delete(id) : s.add(id); return s; });
  const toggleReplies = (id: string) =>
    setExpandedReplies((p) => { const s = new Set(p); s.has(id) ? s.delete(id) : s.add(id); return s; });
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === "Escape") setWriteModalOpen(false); };
  ...
  const closeReplyForm = () => { setShowReplyForm(null); setReplyContent(""); };
  ...
  const handleReplySubmit = async (reviewId: string) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:4000/reviews/${reviewId}/replies`, {
        method: "POST",
        ...
        prev.map((r) =>
          r.id !== reviewId ? r : {
            ...
            replies: [...(r.replies || []), {
              ...
              user: { username: user?.username || "You", avatar_path: user?.avatarUrl || null },
            }],
          }
        )
      );
          ...
          <div className="flex gap-5 sm:gap-8 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-3">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-xs text-white/40">{new Date(info.release_date).getFullYear()}</span>
                {info.content_type === "tv" && info.number_of_seasons > 0 && (
                  <span className="text-xs text-white/40">{info.number_of_seasons} Season{info.number_of_seasons !== 1 ? "s" : ""}</span>
                )}
                {info.content_type === "tv" && info.number_of_episodes > 0 && (
                  <span className="text-xs text-white/40">{info.number_of_episodes} Episodes</span>
                )}
                <div className="h-3 w-px bg-white/10" />
                {info.genres.slice(0, 3).map((g) => (
                  <span key={g.id} className="text-[11px] px-2 py-0.5 rounded-full bg-white/[0.06] border border-white/[0.08] text-white/50">
                    {g.name}
                  </span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-4 flex-wrap pt-1">
                {reviewStats && reviewStats.totalRatings > 0 && (
                  <>
                    <div className="flex items-center gap-2">
                      <RatingArc rating={reviewStats.averageRating} color="#e94f37" />
                      <div>
                        <p className="text-[10px] uppercase tracking-widest text-white/30">Community</p>
                        <p className="text-xs text-white/60">{reviewStats.totalRatings} ratings</p>
                      </div>
                    </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-[#01b4e4]/60">TMDb</p>
                    <p className="text-xs text-white/40">{info.vote_count.toLocaleString()} votes</p>
                  </div>
                </div>
                {topMoods.length > 0 && (
                  <>
                    ...
                    <div className="flex items-center gap-1">
                      {topMoods.slice(0, 3).map((m, i) => (
                        <span key={i} title={`${m.count} votes`} style={{ fontSize: "clamp(1.2rem, 2.5vw, 1.5rem)", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.5))" }}>
                          {m.emoji}
                        </span>
                      ))}
                      <span className="text-[11px] text-white/25 ml-1">Community vibe</span>
                    </div>
                  </>
                )}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[160px] max-w-xs">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 text-xs bg-white/[0.04] border border-white/[0.08] rounded-xl text-white placeholder-white/25 outline-none focus:border-[#e94f37]/35 transition-colors"
            />
          </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredAndSorted.length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex flex-col items-center justify-center py-24 gap-3 rounded-2xl bg-white/[0.03] border border-white/[0.07]"
              >
              ...
              filteredAndSorted.map((review) => {
                ...
                const normalizedRating = review.author_details?.rating != null
                  ? review.author_details.rating / 2
                  : null;
                const accentColor = toAccentColor(review.author_details?.rating);
                const profileHref = review.userId ? `/profile/${encodeURIComponent(review.userId)}` : undefined;
                ...
                return (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    className="group rounded-2xl border border-white/[0.08] bg-white/[0.04] hover:border-white/[0.15] hover:bg-white/[0.06] transition-all duration-200 overflow-hidden"
                  >
                    {/* Top: quote glyph + rating score pill */}
                    <div className="flex items-center justify-between px-5 pt-5 pb-0">
                      <svg className="w-7 h-7 opacity-[0.07] flex-shrink-0" viewBox="0 0 32 32" fill="white" aria-hidden>
                      ...
                      {normalizedRating !== null ? (
                        ...
                        >
                          <svg width="10" height="10" viewBox="0 0 24 24" fill={accentColor!} aria-hidden>
                            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                          </svg>
                          {normalizedRating.toFixed(1)}
                        </div>
                      ) : review.moodEmojis?.[0] ? (
                        <span className="text-xl leading-none">{review.moodEmojis[0]}</span>
                      ) : null}
                    </div>

                    {/* Review text — hero */}
                    <div className="px-5 pt-3 pb-4">
                      <p className="text-sm text-white/70 leading-[1.75] whitespace-pre-wrap">
                    ...
                    {review.replies && review.replies.length > 0 && (
                      <div className="mx-5 mb-4 pl-4 border-l-2 border-white/[0.06] space-y-2.5">
                        {(repliesExpanded ? review.replies : review.replies.slice(0, 2)).map((reply, i) => (
                          <div key={i} className="flex items-start gap-2.5">
                            <div className="w-6 h-6 rounded-full bg-white/[0.06] border border-white/[0.09] flex-shrink-0 flex items-center justify-center text-[10px] font-semibold text-white/45">
                              {reply.user.username.charAt(0).toUpperCase()}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                <span className="text-xs font-medium text-white/65">{reply.user.username}</span>
                                <span className="text-[10px] text-white/25">{formatDate(reply.created_at)}</span>
                                {!banStatus.banned && (
                                  <button
                                    onClick={() =>
                                      showReplyForm?.reviewId === review.id && showReplyForm?.prefill === `@${reply.user.username} — `
                                        ? closeReplyForm()
                        ...
                        {review.replies.length > 2 && (
                          ...
                          >
                            {repliesExpanded ? "Show less" : `+ ${review.replies.length - 2} more repl${review.replies.length - 2 === 1 ? "y" : "ies"}`}
                          </button>
                        )}
                      </div>
                    )}
                      ...
                      {showReplyForm?.reviewId === review.id && (
                        ...
                        >
                          {showReplyForm?.prefill && (
                            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.03] border border-white/[0.06] rounded-xl">
                              <span className="text-xs font-semibold" style={{ color: "#e94f37" }}>{showReplyForm.prefill.trim()}</span>
                          ...
                          <textarea
                            ...
                            rows={3}
                            autoFocus
                            className="w-full bg-white/[0.03] border border-white/[0.08] focus:border-[#e94f37]/35 rounded-xl px-3 py-2.5 text-xs text-white/75 placeholder-white/20 resize-none outline-none transition-colors"
                          />
                    ...
                    <div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-white/[0.07] bg-white/[0.025]">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <AvatarBlock review={review} href={profileHref} size={30} />
                        <div className="min-w-0">
                          {profileHref ? (
                            ...
                            <p className="text-xs font-semibold text-white/75 truncate">{review.author}</p>
                          )}
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-white/25">{formatDate(review.created_at)}</span>
                            {review.moodEmojis?.[0] && normalizedRating !== null && (
                              <span className="text-xs leading-none">{review.moodEmojis[0]}</span>
                            )}
                            {review.author_details?.username && (
                              <span className="text-[10px] text-white/15">@{review.author_details.username}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {/* Reply action */}
                        {banStatus.banned ? (
                          <span className="text-[10px] text-red-400/50">Replies suspended</span>
                        ) : (
                          <button
                            onClick={() =>
                              showReplyForm?.reviewId === review.id ? closeReplyForm() : openReplyForm(review.id)
                            }
                            className="flex items-center gap-1 text-[11px] text-white/25 hover:text-[#e94f37] transition-colors cursor-pointer"
                          >
                            ...
                            {review.replies && review.replies.length > 0 && (
                              <span className="ml-0.5 text-white/15">{review.replies.length}</span>
                            )}
                          </button>
                        )}
                        {review.url && (
                          <Link
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-[10px] font-medium text-white/20 hover:text-[#e94f37] transition-colors"
                          >
                            Original →
                          </Link>
                        )}
              ...
              >
                ...
                <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/[0.07]">
                  <div className="flex items-center gap-3">
                    <svg className="w-7 h-7 opacity-[0.09] flex-shrink-0" viewBox="0 0 32 32" fill="white" aria-hidden>
                      <path d="M10 8C5.6 8 2 11.6 2 16v8h8v-8H4c0-3.3 2.7-6 6-6V8zm12 0c-4.4 0-8 3.6-8 8v8h8v-8h-6c0-3.3 2.7-6 6-6V8z" />
                    </svg>
                    <div>
                      <h2 className="text-sm font-bold text-white leading-none">Write a Review</h2>
                      <p className="text-[11px] text-white/30 mt-1">{info.title}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setWriteModalOpen(false)}
                    ...
                    <div className="flex items-start gap-4 px-5 py-5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 text-[11px] text-white/35">
                          ...
                          Expires in <span className="text-red-400 font-semibold">{banStatus?.timeRemaining || "Unknown"}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
...
function ReviewFormInModal({
  ...
  const moodToEmoji: Record<string, string> = {
    amazing: "🔥", loved: "❤️", enjoyed: "😊", okay: "😐", meh: "😕", disliked: "😞",
  };
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    if (!content.trim() || content.trim().length < 10) { setError("✍️ At least 10 characters needed"); return; }
    setSubmitting(true);
    try {
      const token = sGet("authToken");
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
          ...
          <AnimatePresence mode="popLayout">
            {normalizedDisplay !== null ? (
              <motion.div
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold flex-shrink-0"
                style={{ color: ratingColor!, background: `${ratingColor}18`, border: `1px solid ${ratingColor}35` }}
              >
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-white/[0.07] bg-white/[0.025]">
          <div className="flex items-center gap-2.5">
            ...
            >
              <span className="text-white/60 font-semibold text-[10px]">{author.charAt(0).toUpperCase()}</span>
            </div>
            <div>
        ...
        <div className="flex items-center gap-0.5">
          {Array.from({ length: 5 }).map((_, i) => {
            ...
            return (
              ...
              >
                <Star size={16} className={`transition-colors duration-100 ${active ? "text-yellow-400 fill-yellow-400" : "text-white/20 hover:text-white/35"}`} />
              </button>
            );
          })}
        </div>
...
/* ── Helpers ── */
function RatingArc({ rating, color = "#e94f37", size = 44 }: { rating: number; color?: string; size?: number }) {
  const clamped = Math.max(0, Math.min(10, rating));
  const r = size * 0.28, circ = 2 * Math.PI * r;
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth="3" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={circ * (1 - clamped / 10)}
          style={{ transform: "rotate(-90deg)", transformOrigin: "center" }}
        />
      </svg>
      <span className="absolute inset-0 flex items-center justify-center font-bold text-white" style={{ fontSize: size < 36 ? "0.55rem" : "0.65rem" }}>
        {clamped.toFixed(1)}
      </span>
    </div>
  );
...
function AvatarBlock({ review, href, size = 44 }: { review: Review; href?: string; size?: number }) {
  const avatarSrc = (() => {
  ...
  const initials = (review.author || "A").split(" ").map((s) => s[0]?.toUpperCase() ?? "").slice(0, 2).join("");
  const avatar = (
    <div
    ...
    >
      {avatarSrc ? (
        ...
        <img src={avatarSrc} alt={review.author} width={size} height={size} referrerPolicy="no-referrer" className="object-cover w-full h-full" />
      ) : (
        <span className="text-white/50 font-semibold" style={{ fontSize: size < 36 ? "10px" : "13px" }}>{initials}</span>
      )}
    </div>
  );
  if (!href) return avatar;
  return (
    <Link href={href} aria-label={`View ${review.author}'s profile`} className="group rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[#e94f37]">
      {avatar}
    </Link>
  );
}
