...
/** Your server base (unchanged) */
const API_BASE = process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000";

/** TMDB auth: prefer v4 Bearer; fall back to v3 api_key if provided */
...
type Item =
  | ({ kind: "movie" } & TmdbMovie)
  | ({ kind: "tv" } & TmdbTv);

/* -------------------- Helpers -------------------- */

/** Build TMDB URL handling api_key fallback cleanly */
...
/** Concurrency limiter for nicer UX & to avoid bursty requests */
...
export default function WatchlistPage() {
  ...
  /* 2) For any IDs we got, fetch TMDB details (movies & tv) */
  useEffect(() => {
    let alive = true;
    (async () => {
      ...
      // Fetch with modest concurrency to keep things smooth
    ...
    <main className="min-h-screen bg-gradient-to-b from-black via-slate-900 to-black text-white">
      <div className="mx-auto w-full max-w-6xl px-6 py-10">
        ...
        <div className="mb-6 flex flex-wrap gap-2 text-xs">
          ...
          <Badge label={`Movie IDs: ${movieIds.length}`} ok />
          <Badge label={`TV IDs: ${tvIds.length}`} ok />
          <Badge label={`TMDB auth: ${TMDB_READ_TOKEN ? "v4" : TMDB_API_KEY ? "v3" : "missing"}`} ok={!!(TMDB_READ_TOKEN || TMDB_API_KEY)} />
              ...
              <Section title={`Movies (${movieItems.length})`}>
                <Grid>
                  {movieItems.map((m) => (
                    <Card
                      key={`m-${m.id}`}
                      href={`/movie/${m.id}`} // keep your existing movie page pathing
              ...
              <Section title={`Series (${tvItems.length})`}>
                <Grid>
                  {tvItems.map((t) => (
                    <Card
                      key={`t-${t.id}`}
                      href={`/tv/${t.id}`} // keep your existing tv page pathing
  ...
  const body = (
    <div className="group relative overflow-hidden rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition">
      ...
      <div className="p-3">
        ...
        {overview ? (
          <p className="text-xs text-white/70 mt-2 line-clamp-3">{overview}</p>
        ) : null}
      </div>
    </div>
  );
