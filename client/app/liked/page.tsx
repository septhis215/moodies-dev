...
export default function LikedPage() {
  ...
  /* 1) Load liked list (IDs) from server */
  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true);
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-8 sm:py-16 lg:px-12">
        ...
        <div className="mt-10 mb-12 relative overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-end justify-between gap-6">
            ...
            <div>
              <div className="mb-2">
                <div className="w-8 h-0.5 bg-pink-500 mb-2" />
            ...
            {(movieItems.length > 0 || tvItems.length > 0) && (
              <div className="relative w-full sm:w-64 flex-shrink-0">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-2.5 text-sm rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/20 outline-none focus:border-pink-500/50 focus:bg-white/[0.06] transition-all"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
        ...
        {loading && (
          <div className="text-white/80">Loading your likes…</div>
        )}

        {!loading && err && (
          <EmptyState title="Couldn't load your likes" note={err} />
        )}
        ...
        {!loading &&
          ...
          tvIds.length === 0 && (
            <EmptyState
              title="No likes yet"
              note="Like a movie or series from the feed or detail pages."
            />
          )}
                ...
                <Grid>
                  {filteredMovies.slice(0, pageSize * moviePage).map((m) => (
                    <Card
                      ...
                      onRemove={() => removeFromLiked("movie", m.id)}
                    />
                  ))}
                </Grid>
                ...
                <Grid>
                  {filteredTv.slice(0, pageSize * tvPage).map((t) => (
                    <Card
                      ...
                      onRemove={() => removeFromLiked("tv", t.id)}
                    />
                  ))}
                </Grid>
...
function FilterBar({
  ...
  const dotClass = accent === "pink" ? "bg-pink-400" : "bg-[rgb(233,79,55)]";
    ...
    <div className="group relative h-full">
      <Link href={href} className="block h-full">
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-zinc-900 to-zinc-950 shadow-xl ring-1 ring-white/5 aspect-[2/3]">
          ...
          <div className="absolute top-0 left-3 drop-shadow-lg">
            ...
            >
              ...
              />
              <path d="M2 0H24V32L13 25L2 32V0Z" fill="rgb(236,72,153)" />
              <path d="M2 0H13V32L2 32V0Z" fill="rgba(255,255,255,0.07)" />
              <path d="M2 0H24V2H2V0Z" fill="rgba(255,255,255,0.2)" />
              {/* Heart icon */}
              <path
                d="M13 20s-5-3.5-5-7a3 3 0 0 1 5-2.24A3 3 0 0 1 18 13c0 3.5-5 7-5 7z"
                fill="white"
                opacity="0.9"
              />
            </svg>
        ...
        <div className="mt-3 px-1">
          <h4 className="font-bold text-sm sm:text-base line-clamp-2 group-hover:text-pink-400 transition-colors leading-tight text-white">
            {title}
          </h4>
