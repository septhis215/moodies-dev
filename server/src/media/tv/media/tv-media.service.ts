...
export class TvMediaService {
    private readonly logger = new Logger(TvMediaService.name);

    constructor(private readonly client: TvTmdbClientService) { }

    async images(id: number, type: string = 'tv') {
    ...
    async videos(id: number, type: string = 'tv') {
            ...
            for (let p = 1; p <= 3; p++) {
                ...
                for (const item of results) {
                    ...
                    const reviewPromises = Array.from({ length: 3 }, (_, i) =>
                        this.client.tmdb(`tv/${item.id}/reviews?language=en-US&page=${i + 1}`),
                    );
