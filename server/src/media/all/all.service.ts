...
export class AllService implements OnModuleInit {
    ...
    // --- Trailers ---
    getTrailers(limit = 30) { return this.trailersService.getTrailers(limit); }
    getUpcomingTrailers(limit = 30) { return this.trailersService.getUpcomingTrailers(limit); }
    getTrailersForItems(items: { type: 'movie' | 'tv', id: number }[]) {
        return this.trailersService.getTrailersForItems(items);
    }
    getMovieVideos(id: number) { return this.trailersService.getMovieVideos(id); }
    getTvVideos(id: number) { return this.trailersService.getTvVideos(id); }

    // --- Upcoming Feed ---
    getUpcomingFeeds(page = 1, limit = 35) { return this.upcomingFeedService.getUpcomingFeeds(page, limit); }

    // --- Recommendations ---
    getSmartRecommendations(type: 'movie' | 'tv', id: number, limit = 10, minRequired = 3) {
        return this.recommendationsService.getSmartRecommendations(type, id, limit, minRequired);
    }
    ...
    // --- People ---
    getPeople(limit = 30) { return this.peopleService.getPeople(limit); }

    // --- Search ---
    getSearchSuggestions(query: string, limit = 6) {
        return this.searchService.getSearchSuggestions(query, limit);
    }
    getTrendingSearchTerms() { return this.searchService.getTrendingSearchTerms(); }

    // --- Reviews ---
    getTrendingReviews(limit = 40) { return this.reviewsService.getTrendingReviews(limit); }

    // --- Images ---
    images(id: number, type: string) { return this.imagesService.images(id, type); }
}